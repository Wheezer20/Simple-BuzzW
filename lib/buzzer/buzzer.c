...
void enableBuzzer()
{
   ...
   PORTD |= ( 1 << PD3 ); // Initialize buzzer to OFF state (HIGH = off for this buzzer)
}

void disableBuzzer()
{
   PORTD |= ( 1 << PD3 ); // Turn buzzer OFF (HIGH = off)
}
...
void playTone( float frequency, uint32_t duration )
{
   ...
   // Ensure buzzer is completely off after playing tone
   PORTD |= ( 1 << PD3 );
}